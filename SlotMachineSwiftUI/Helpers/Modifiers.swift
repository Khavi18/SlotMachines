...
struct ShadowModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .shadow(color: Color(blackT),radius: 0, x: 0, y: 6)
    }
}
...
struct ScoreNumberModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .shadow(color: Color(blackT), radius: 0, x: 0, y: 3)
            .layoutPriority(1)
    }
}
...
struct BetCapsuleModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            ...
            .background(
                Capsule()
                    .fill(LinearGradient(gradient: Gradient(colors: [Color(pink), Color(purple)]), startPoint: .bottom, endPoint: .top))
                    .modifier(ShadowModifier())
            )
    }
}
